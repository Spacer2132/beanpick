...
for (const note of ['헤이즐넛', '견과류', '초콜릿']) {
  if (!rubiaGalleryNotes.includes(note)) {
    throw new Error(`루비아 갤러리 카드 노트를 읽지 못했습니다: ${rubiaGalleryNotes.join(', ') || '(없음)'}`);
  }
}
// 단맛·신맛은 신맛↔단맛 축 라벨이라 향미 노트로 수집되면 안 된다.
const axisLabelNotes = _test.extractFlavorNotesAnywhere('단맛 ●●●○○ 신맛 ●○○○○ 헤이즐넛');
if (axisLabelNotes.includes('단맛') || axisLabelNotes.includes('신맛')) {
  throw new Error(`축 라벨을 향미 노트로 수집하면 안 됩니다: ${axisLabelNotes.join(', ')}`);
}
