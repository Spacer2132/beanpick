...
// 스토어별 주소는 비로그인 사파리에서 그대로 열린다(2026-09-10 실기 확인). /main/products/만 로그인으로 튕겨 검색 우회가 필요하다.
...
expect(
  core.resolveProductOpenUrl('https://smartstore.naver.com/undercrema/category/abc123', smartStoreProduct)
    === 'https://smartstore.naver.com/undercrema/category/abc123',
  '스마트스토어 카테고리(원두 목록) 주소도 그대로 열려야 합니다',
  core.resolveProductOpenUrl('https://smartstore.naver.com/undercrema/category/abc123', smartStoreProduct),
);
expect(
  core.resolveProductOpenUrl('https://smartstore.naver.com/main/products/9547825639', smartStoreProduct)
    === 'https://search.shopping.naver.com/search/all?query=' + encodeURIComponent('말릭커피 프루티블렌드'),
  '/main/products/ 주소는 로그인으로 튕기므로 로스터리명+상품명 검색으로 바뀌어야 합니다',
  core.resolveProductOpenUrl('https://smartstore.naver.com/main/products/9547825639', smartStoreProduct),
);
expect(
