...
const { extractRoastLevelFromDetail } = require('../roastLevel.cjs');
...
function parseCafe24DetailInfo(html) {
  ...
  // FIX-3: 상세 텍스트에서 로스팅 추출 (로스팅 표기가 붙은 값만. 없으면 빈 문자열 — 호출자가 기본값 처리)
  const roastLevel = extractRoastLevelFromDetail(`${description || ''}\n${lines.join('\n')}`);
