...
// 상세 텍스트용 엄격 추출. 상세 본문에는 "Dark Chocolate", "미디엄 바디", "Light body", "라이트한 산미"처럼
// 컵노트·바디감 설명이 흔해 단독 '다크/라이트/미디엄'을 로스팅으로 보면 오탐이 난다.
// 그래서 로스팅 표기(로스트·배전·로스팅·roast)가 붙은 표현만 인정한다.
const DETAIL_MARKER_PATTERN = /로스트|배전|roast/i;
const DETAIL_ROAST_PATTERNS = ROAST_PATTERNS.filter(([rx]) => DETAIL_MARKER_PATTERN.test(rx.source));
// "로스팅 포인트: 미디엄", "Roast level: Medium-Dark"처럼 표기와 값 사이에 포인트·레벨·단계가 끼는 경우
const DETAIL_LABELED_RX = new RegExp('(?:로스팅|배전도|볶음도|roast(?:ing)?)\\s*(?:포인트|레벨|단계|정도|point|level)\\s*[:：]?\\s*' + EXPLICIT_VALUE_SRC, 'gi');

function extractRoastLevelFromDetail(text) {
  if (!text) return '';
  const work0 = maskFalsePositives(text);
  const explicit = extractExplicit(work0);
  if (explicit) return explicit;
  ...
  let lm;
  DETAIL_LABELED_RX.lastIndex = 0;
  // eslint-disable-next-line no-cond-assign
  while ((lm = DETAIL_LABELED_RX.exec(work0)) !== null) {
    const level = mapExplicitValue(lm[0].replace(/^(?:로스팅|배전도|볶음도|roast(?:ing)?)\s*(?:포인트|레벨|단계|정도|point|level)/i, ''));
    if (level) labeled.add(level);
  }
  if (labeled.size === 1) return [...labeled][0];
  if (labeled.size > 1) return '';
  let work = work0;
  const found = new Set();
  for (const [rx, level] of DETAIL_ROAST_PATTERNS) {
    rx.lastIndex = 0;
    let m;
    // eslint-disable-next-line no-cond-assign
    while ((m = rx.exec(work)) !== null) {
